...
Проект-эксперимент с попыткой убрать подстроечные конденсаторы в цифровых сигнальных цепях для компьютера **Дельта 128К** (на БМК). Устройство реализовано на ПЛИС (CPLD) и размещается на дополнительной плате, подключаемой к плате компьютера. Репозиторий содержит полный набор файлов для прошивки ПЛИС (epm7064, Quartus) и печатной платы (Altium Designer).
Также есть схема в pdf и собранный файл .pof для моей сборки (паук версии выпуска 9404)


При сборке реплики компьютера Дельта-128к на БМК выяснилось, что для обеспечения его стабильной работы используют RC цепочки в цифровых частях схемы.
- Между тактовым генератором и входом клока БМК
- Между БМК и памятью в сигналах RAS, CAS0, CAS1
- Также есть "иголка" в цепи wait, но она ни на что не влияет.


На текущий момент (18.02.2026) макет такой платы позволяет работать реплике компьютера, есть некоторые не изученные моменты, но в общем он хотя бы запускается.
